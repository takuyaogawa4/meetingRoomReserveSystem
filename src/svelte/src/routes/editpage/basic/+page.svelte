<script lang="ts">
    ...
    // 予約確認
    // async function handleReserveCheck(){
...
<!-- 戻るボタン -->
  <a class="btn w-32 h-10  bg-base-200 text-ghost-content hover:bg-base-300 mt-10"style="float: right;" href="./" >戻る</a>
...
<div class="modal" role="dialog">
  ...
  <br/>
    ...
    <div  class="modal-action">
      <!-- 確定ボタン -->
      <button class="col-start-5  btn w-32 text-lg" on:click={() => (insertReserve(reserveData),popUp2=false)}>確定</button>
      <!-- //Todo 送信関数を入れ込む↑ -->
      <!-- キャンセルボタン -->
